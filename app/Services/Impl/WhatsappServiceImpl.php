...
class WhatsappServiceImpl implements WhatsappService
{
    ...
    private function sendRequest($route, $data): object
    {
        try {
            ...
            Log::warning('WA backend returned an invalid response', [
                'route' => $route,
                'http_status' => $results->status(),
                'body' => $body,
            ...
            return (object) [
                'status' => false,
                'success' => false,
                'message' => 'WA backend returned an invalid or empty response.',
                'http_status' => $results->status(),
                'raw_response' => $body,
            ];
        } catch (\Throwable $th) {
            Log::error('WA backend request failed', [
                'route' => $route,
