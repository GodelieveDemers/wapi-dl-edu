...
class UpdateController extends Controller
{
    ...
    protected function updateBaseUrl(): string
    {
        return rtrim(env('WAPI_UPDATE_BASE_URL', 'https://update.khanifmaksum.my.id/mpwa'), '/');
    }

    protected function updateUrl(string $path, array $query = []): string
    {
        $url = $this->updateBaseUrl() . '/' . ltrim($path, '/');
        if (!empty($query)) {
    ...
    public function checkUpdate(Request $request)
	{
		...
		$checkUrl = $this->updateUrl('tools/check.php', [
            'v' => $currentVersion,
    ...
    public function installUpdate(Request $request)
	{
		...
		if($beforeInput === '1'){
			$beforeCommandUrl = $this->updateUrl("tools/{$newVersion}/command-before.txt", ['rand' => $randomNumber]);
		...
		if ($sslInput === 'ssl') {
			...
			$newVersionUrl = $this->updateUrl("repo/update/linux/{$newVersion}/update-ssl.zip", ['rand' => $randomNumber]);
		}else{
			$newVersionUrl = $this->updateUrl("repo/update/linux/{$newVersion}/update.zip", ['rand' => $randomNumber]);
		}
		...
		if ($response != "") {
			...
			if($afterInput === '1'){
				$afterCommandUrl = $this->updateUrl("tools/{$newVersion}/command-after.txt", ['rand' => $randomNumber]);
