    ...
    public function update(Request $request, Contact $contact)
    {
        try {
            if ($contact->user_id !== $request->user()->id) {
                return response()->json(['error' => true, 'msg' => __('Something errors!')], 403);
            }

            $request->validate([
                'name' => ['required'],
                'number' => ['required', 'min:8', 'max:13'],
            ]);

            $exists = $request->user()->contacts()
                ->where('number', $request->number)
                ->whereTagId($contact->tag_id)
                ->where('id', '!=', $contact->id)
                ->first();

            if ($exists) {
                return response()->json(['error' => true, 'msg' => __('Number already exists in this phonebook!')]);
            }
