...
class ApiController extends Controller
{
    ...
    private function resolveApiUserByToken(string $apiKey): ?User
    {
        $apiKey = trim($apiKey);
        ...
        $device = Device::with('user')->where('api_token', $apiKey)->first();

        return $device?->user;
    }

    private function resolveIncomingApiToken(Request $request): string
    {
    ...
    {
        $apiToken = $this->resolveIncomingApiToken($request);
        if ($apiToken === '' || !$request->has('device')) return $this->sendFailResponse('Invalid parameters!');
        $user = $this->resolveApiUserByToken($apiToken);
    ...
    {
        $apiToken = $this->resolveIncomingApiToken($request);
        if ($apiToken === '') return $this->sendFailResponse(__('Invalid parameters!'));
		try {
			$user = $this->resolveApiUserByToken($apiToken);
			if ($user->level != 'admin') {
                return response()->json(
