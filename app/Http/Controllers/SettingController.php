...
class SettingController extends Controller
{
	...
	public function setMenuVisibility(Request $request)
	{
		$showPlans = $request->has('show_user_plans_menu') ? 'true' : 'false';
		setEnv('MPWA_SHOW_USER_PLANS_MENU', $showPlans);
		Artisan::call('optimize:clear');

		return back()->with('alert', [
			'type' => 'success',
			'msg' => __('Menu visibility updated successfully.')
		]);
	}

	public function setBranding(Request $request)
	{
		$data = $request->validate([
			'site_name' => ['required', 'string', 'max:120'],
			'header_side' => ['required', 'string', 'max:120'],
			'footer_name' => ['required', 'string', 'max:120'],
			'logo' => ['nullable', 'image', 'mimes:png,jpg,jpeg,webp,svg', 'max:2048'],
			'login_illustration' => ['nullable', 'image', 'mimes:png,jpg,jpeg,webp,svg', 'max:4096'],
		]);

		$this->setQuotedEnv('MPWA_SITE_NAME', $data['site_name']);
		$this->setQuotedEnv('MPWA_HEADER_SIDE', $data['header_side']);
		$this->setQuotedEnv('MPWA_FOOTER_NAME', $data['footer_name']);
		...
		if ($request->hasFile('logo')) {
			...
			$this->setQuotedEnv('MPWA_LOGO_PATH', 'uploads/branding/' . $filename);
		}
		...
		if ($request->hasFile('login_illustration')) {
			...
			$extension = $request->file('login_illustration')->getClientOriginalExtension();
			$filename = 'login-illustration.' . strtolower($extension);
			$request->file('login_illustration')->move($directory, $filename);
			$this->setQuotedEnv('MPWA_LOGIN_ILLUSTRATION_PATH', 'uploads/branding/' . $filename);
		}
		...
		return back()->with('alert', [
			'type' => 'success',
			'msg' => __('Branding display updated successfully.')
		]);
	}
	...
	private function setQuotedEnv(string $key, string $value): void
	{
		$path = base_path('.env');
