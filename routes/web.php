...
use App\Http\Controllers\Admin\EsmsIntegrationController;
use App\Http\Controllers\Admin\LiveChatMonitorController;
...
use App\Http\Controllers\ChatSyncController;
...
use App\Http\Controllers\WhatsAppChannelController;
use App\Http\Controllers\MetaDataDeletionController;
...
Route::view('/privacy', 'theme::pages.legal.privacy')->name('legal.privacy');
Route::view('/terms', 'theme::pages.legal.terms')->name('legal.terms');
Route::get('/meta/data-deletion', function () {
    return view('theme::pages.legal.data-deletion-status', ['request' => null, 'code' => 'Gunakan POST signed_request dari Meta untuk callback penghapusan data.']);
})->name('meta.data-deletion.info');
Route::get('/meta/data-deletion/status/{code}', [MetaDataDeletionController::class, 'status'])->name('meta.data-deletion.status');

Route::group(['prefix' => LaravelLocalization::setLocale()], function() {

	Route::view('/privacy', 'theme::pages.legal.privacy')->name('localized.legal.privacy');
	Route::view('/terms', 'theme::pages.legal.terms')->name('localized.legal.terms');
	Route::get('/meta/data-deletion', function () {
		return view('theme::pages.legal.data-deletion-status', ['request' => null, 'code' => 'Gunakan POST signed_request dari Meta untuk callback penghapusan data.']);
	})->name('localized.meta.data-deletion.info');
	Route::get('/meta/data-deletion/status/{code}', [MetaDataDeletionController::class, 'status'])->name('localized.meta.data-deletion.status');
	Route::get('/', function()
	{
	...
	Route::middleware('auth', '2fa')->group(function (){
		...
		Route::post('/whatsapp-channels/cloud',[WhatsAppChannelController::class,'storeCloud'])->name('whatsapp.channels.cloud.store');
		Route::delete('/whatsapp-channels/cloud/{id}',[WhatsAppChannelController::class,'destroyCloud'])->name('whatsapp.channels.cloud.destroy');
		Route::post('/whatsapp-channels/embedded/callback',[WhatsAppChannelController::class,'embeddedCallback'])->name('whatsapp.channels.embedded.callback');
		...
		Route::post('/devices/regenerate-token',[DevicesController::class,'regenerateApiToken'])->name('regenerateDeviceApiToken');
		...
		Route::put('/contact/update/{contact:id}',[ContactController::class,'update'])->name('contact.update');
  ...
  Route::get('/plans',[UserPlansController::class,'index'])->name('user.plans.index');
	  ...
	  Route::get('/chat/messages/{session}', [ChatController::class, 'messages'])->name('chat.messages')->middleware('permissions');
          Route::get('/chat/sync', [ChatSyncController::class, 'index'])->name('chat.sync')->middleware('permissions');
          Route::post('/chat/mark-read/{session}', [ChatSyncController::class, 'markRead'])->name('chat.markRead')->middleware('permissions');
  ...
  Route::post('/settings/branding', [SettingController::class, 'setBranding'])->name('settings.branding')->middleware('admin');
  Route::post('/settings/registration', [SettingController::class, 'setRegistration'])->name('settings.registration')->middleware('admin');
  Route::post('/settings/menu-visibility', [SettingController::class, 'setMenuVisibility'])->name('settings.menu-visibility')->middleware('admin');
	  ...
	  Route::get('/admin/esms-integration', [EsmsIntegrationController::class, 'index'])->name('admin.esms-integration')->middleware('admin');
	  Route::post('/admin/esms-integration', [EsmsIntegrationController::class, 'update'])->name('admin.esms-integration.update')->middleware('admin');
	  Route::get('/admin/live-chat-monitor', [LiveChatMonitorController::class, 'index'])->name('admin.live-chat-monitor');
	  Route::get('/admin/live-chat-monitor/data', [LiveChatMonitorController::class, 'data'])->name('admin.live-chat-monitor.data');
	  ...
	  if (class_exists(\App\Http\Controllers\StripeController::class)) {
	  ...
	  Route::get('/analytics-reports',[AnalyticsReportController::class,'index'])->name('analytics.reports');
