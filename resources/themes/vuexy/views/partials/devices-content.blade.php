@php $embedded = $embedded ?? false; @endphp
	...
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb breadcrumb-custom-icon">
			<li class="breadcrumb-item">
				<a href="{{ route('whatsapp.channels.index') }}">Channel WhatsApp</a>
				<i class="breadcrumb-icon icon-base ti tabler-chevron-right align-middle icon-xs"></i>
			</li>
			<li class="breadcrumb-item active">WhatsApp Web / QR</li>
		</ol>
	</nav>
	<!--end breadcrumb-->
	@endif
				...
				<div class="card-header">
					<div class="d-flex flex-wrap align-items-center gap-2">
						<div>
							...
							<p class="text-muted mb-0 small">Kelola nomor WhatsApp Web/Baileys. Nomor ini tetap digunakan untuk mode QR, sementara Cloud API resmi tersedia di tab sebelah.</p>
						</div>
						<div class="ms-auto d-flex flex-wrap gap-2">
							@if (! $embedded)
								<a href="{{ route('whatsapp.channels.index') }}" class="btn btn-outline-secondary btn-sm"><i class="ti tabler-arrow-left me-1"></i> Kembali ke Channel</a>
							@endif
				...
				<div class="table-responsive mb-4">
					<table class="table datatable-project table-sm">
						<thead class="border-top">
							<th>{{__('Number')}}</th>
							<th class="text-nowrap">{{__('API Token')}}</th>
						...
						<tbody>
							@if ($numbers->total() == 0)
							<x-no-data colspan="7" text="No Device added yet" />
							@endif
							...
							<tr>
								...
								<td class="text-nowrap">
									<div class="d-flex align-items-center gap-2">
										<input type="text" class="form-control form-control-sm" value="{{ $number['api_token'] }}" readonly style="min-width: 220px;">
										<button type="button" class="btn btn-outline-secondary btn-sm copy-device-token" data-token="{{ $number['api_token'] }}">
						...
						<div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
							<div class="flex-grow-1">
								<div class="d-flex align-items-center gap-2 mb-2">
									<span class="badge bg-label-dark rounded-2 p-2"><i class="ti tabler-key"></i></span>
									<div class="fw-medium">{{ __('API Token') }}</div>
								</div>
								<input type="text" class="form-control" value="{{ $number['api_token'] }}" readonly>
								<small class="text-muted d-block mt-2">{{ __('Use this token for this phone/device only. Legacy user api_key remains supported for backward compatibility.') }}</small>
							</div>
							<div class="d-flex flex-column gap-2">
								<button type="button" class="btn btn-outline-secondary copy-device-token" data-token="{{ $number['api_token'] }}">
									<i class="ti tabler-copy me-1"></i> {{ __('Copy Token') }}
								</button>
								<form action="{{ route('regenerateDeviceApiToken') }}" method="POST">
									@csrf
									<input type="hidden" name="deviceId" value="{{ $number['id'] }}">
									<button type="submit" class="btn btn-outline-danger" onclick="return confirm('{{ __('Regenerate API token for this device? Old integrations using the old token will stop working.') }}')">
										<i class="ti tabler-refresh me-1"></i> {{ __('Regenerate Token') }}
									</button>
								</form>
							</div>
						</div>
	...
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			...
			document.querySelectorAll('.copy-device-token').forEach(function (button) {
				button.addEventListener('click', async function () {
					...
					try {
						...
						window.prompt('Copy API token:', token);
					}
				});
			});
