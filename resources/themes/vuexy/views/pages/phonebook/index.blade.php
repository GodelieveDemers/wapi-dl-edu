<x-layout-dashboard title="{{__('Phone book')}}">
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="editContactCanvas" aria-labelledby="editContactLabel">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="editContactLabel">{{__('Edit Contact')}}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="{{__('Cancel')}}"></button>
        </div>
        <div class="offcanvas-body">
            <form class="edit-contact-form" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" class="edit-contact-id" name="contact_id" value="">
                <label for="edit_contact_name" class="form-label">{{__('Name')}}</label>
                <input type="text" name="name" class="form-control edit-contact-name" id="edit_contact_name" required>
                <label for="edit_contact_number" class="form-label mt-2">{{__('Number')}}</label>
                <input type="number" name="number" class="form-control edit-contact-number" id="edit_contact_number" required>
                <div class="d-flex justify-content-end mt-3">
                    <button type="button" class="btn btn-sm btn-secondary me-2" data-bs-dismiss="offcanvas">{{__('Cancel')}}</button>
                    <button type="submit" name="submit" class="btn btn-sm btn-primary edit-contact-submit">{{__('Update')}}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script src="{{ asset('js/phonebook.js') }}?v={{config('app.version')}}-edit-contact-20260523"></script>
    <script>
    window.openPhonebookEditContact = function(element) {
        var button = $(element);
        $('.edit-contact-id').val(button.data('contact-id'));
        $('.edit-contact-name').val(button.data('contact-name'));
        $('.edit-contact-number').val(button.data('contact-number'));

        var offcanvas = document.getElementById('editContactCanvas');
        if (window.bootstrap && bootstrap.Offcanvas && offcanvas) {
            bootstrap.Offcanvas.getOrCreateInstance(offcanvas).show();
            return;
        }

        $('#editContactCanvas').modal('show');
    };

    window.openEditContact = function(id, element) {
        window.openPhonebookEditContact(element);
    };
	...
	window.addEventListener('load', function() {
      ...
      $.fn.modal=function(action){
        var id=this.attr('id');
        if(id==='addContact'||id==='editContactCanvas'||id==='importContacts'){
