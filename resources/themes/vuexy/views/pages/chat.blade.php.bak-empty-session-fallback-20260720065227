...
<script>
  ...
  const ChatSyncUrl = "{{ route('chat.sync') }}";
  const MarkReadUrl = "{{ route('chat.markRead', ['session' => '__SESSION__']) }}";
  Lang['Audio'] = '{{__("Audio")}}';
  Lang['Video'] = '{{__("Video")}}';
...
<script type="module" src="{{ asset('js/emoji/picker.min.js') }}?v={{config('app.version')}}"></script>
	<script src="{{ asset('js/app-chat.js') }}?v={{config('app.version')}}-livechat-sync-20260720c"></script>
@endif
</x-layout-dashboard>
