...
<div class="card mb-4">
    <div class="card-body">
        <h5 class="mb-3"><i class="ti tabler-layout-sidebar me-1"></i> {{__('User Menu Visibility')}}</h5>
        <form action="{{ route('settings.menu-visibility') }}" method="POST" class="row g-3">
            @csrf
            <div class="col-md-12 d-flex align-items-center justify-content-between">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="showUserPlansMenuSwitch" name="show_user_plans_menu" value="1" {{ config('config.show_user_plans_menu', 'true') == 'true' ? 'checked' : '' }}>
                    <label class="form-check-label ms-2" for="showUserPlansMenuSwitch">{{__('Show Plans menu for users')}}</label>
                </div>
                <button type="submit" class="btn btn-sm btn-outline-primary">
                    <i class="ti tabler-check me-1"></i> {{__('Save')}}
                </button>
            ...
            <div class="col-12">
                <small class="text-muted">{{__('Uncheck this option to hide the Plans menu from non-admin users.')}}</small>
            </div>
        </form>
    </div>
</div>
...
<div class="card mb-4">
    <div class="card-body">
        <h5 class="mb-3"><i class="ti tabler-id-badge-2 me-1"></i> {{__('Branding Display')}}</h5>
        <form action="{{ route('settings.branding') }}" method="POST" enctype="multipart/form-data" class="row g-3">
            @csrf
            <div class="col-md-4">
                <label class="form-label">{{__('Site Name')}}</label>
                <input type="text" name="site_name" class="form-control" value="{{ config('config.site_name') }}" required>
                <small class="form-text text-muted">{{__('Shown in page titles and browser tabs.')}}</small>
            </div>
            <div class="col-md-4">
                <label class="form-label">{{__('Header Side')}}</label>
                <input type="text" name="header_side" class="form-control" value="{{ config('config.header_side') }}" required>
                <small class="form-text text-muted">{{__('Shown in footer copyright text.')}}</small>
            </div>
            <div class="col-md-4">
                <label class="form-label">{{__('Footer Name')}}</label>
                <input type="text" name="footer_name" class="form-control" value="{{ config('config.footer_name') }}" required>
                <small class="form-text text-muted">{{__('Used as the short application name.')}}</small>
            </div>
            <div class="col-md-8">
                <label class="form-label">{{__('Logo')}}</label>
                <input type="file" name="logo" class="form-control" accept=".png,.jpg,.jpeg,.webp,.svg,image/png,image/jpeg,image/webp,image/svg+xml">
                <small class="form-text text-muted">{{__('Upload PNG, JPG, WEBP, or SVG. Maximum size 2 MB.')}}</small>
            </div>
            <div class="col-md-4">
                <label class="form-label">{{__('Current Logo')}}</label>
                <div class="border rounded p-3 d-flex align-items-center" style="min-height: 68px;">
                    @if(config('config.logo_path'))
                        <img src="{{ url('/' . ltrim(config('config.logo_path'), '/')) }}?v={{ @filemtime(public_path(config('config.logo_path'))) ?: time() }}" alt="Logo" style="max-height: 42px; max-width: 180px; width: auto;">
                    @else
                        <span class="text-muted">{{__('Default SVG logo is being used.')}}</span>
                    @endif
                </div>
            </div>
            <div class="col-md-8">
                <label class="form-label">{{__('Login Illustration')}}</label>
                <input type="file" name="login_illustration" class="form-control" accept=".png,.jpg,.jpeg,.webp,.svg,image/png,image/jpeg,image/webp,image/svg+xml">
                <small class="form-text text-muted">{{__('Shown on the left side of the login page. Maximum size 4 MB.')}}</small>
            </div>
            <div class="col-md-4">
                <label class="form-label">{{__('Current Login Illustration')}}</label>
                <div class="border rounded p-3 d-flex align-items-center justify-content-center" style="min-height: 96px;">
                    @if(config('config.login_illustration_path'))
                        <img src="{{ url('/' . ltrim(config('config.login_illustration_path'), '/')) }}?v={{ @filemtime(public_path(config('config.login_illustration_path'))) ?: time() }}" alt="Login Illustration" style="max-height: 72px; max-width: 180px; width: auto;">
                    @else
                        <span class="text-muted">{{__('Default login illustration is being used.')}}</span>
                    @endif
                </div>
            </div>
            <div class="col-12 d-flex justify-content-end">
                <button type="submit" class="btn btn-sm btn-outline-primary">
                    <i class="ti tabler-check me-1"></i> {{__('Save Branding')}}
                </button>
            </div>
        </form>
    </div>
