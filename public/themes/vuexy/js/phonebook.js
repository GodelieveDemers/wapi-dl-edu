...
function openEditContact(id, element) {
  if (window.openPhonebookEditContact) {
    window.openPhonebookEditContact(element);
    return;
  }

  const button = $(element);
  $(".edit-contact-id").val(button.data("contact-id") || id);
...
window.openPhonebookEditContact = window.openPhonebookEditContact || function (element) {
  const button = $(element);
...
$(document).on("click", ".edit-contact-btn", function (e) {
  e.preventDefault();
  if (window.openPhonebookEditContact) {
    window.openPhonebookEditContact(this);
  } else {
    openEditContact($(this).data("contact-id"), this);
  }
});
...
$(".edit-contact-form").submit(function (e) {
  ...
  if (!contactId) {
    notyf.error("Please select contact");
    return;
  }
