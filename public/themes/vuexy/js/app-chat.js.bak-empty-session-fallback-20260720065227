...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let liveChatSyncInFlight = false;
  let liveChatSyncQueued = false;
  let liveChatSyncTimer = null;

  function contactDisplayMessage(message) {
    return typeof Lang !== 'undefined' && Lang[message] ? Lang[message] : (message || '');
  }

  function ensureUnreadBadge(contact) {
    let badge = contact.querySelector('.live-chat-unread-badge');
    if (badge) return badge;
    const timeEl = contact.querySelector('.chat-contact-list-item-time');
    badge = document.createElement('span');
    badge.className = 'badge rounded-pill bg-primary live-chat-unread-badge ms-2';
    badge.style.minWidth = '1.35rem';
    if (timeEl && timeEl.parentNode) timeEl.parentNode.appendChild(badge);
    return badge;
  }

  function updateUnreadBadge(contact, count) {
    if (!contact) return;
    count = Number(count || 0);
    contact.dataset.unreadCount = String(count);
    const badge = contact.querySelector('.live-chat-unread-badge');
    if (count < 1) {
      if (badge) badge.remove();
      return;
    }
    const nextBadge = badge || ensureUnreadBadge(contact);
    nextBadge.textContent = count > 99 ? '99+' : String(count);
  }

  function markSessionRead(sessionId) {
    if (!sessionId || typeof MarkReadUrl === 'undefined') return;
    const contact = document.querySelector(`[data-session-id="${sessionId}"]`);
    updateUnreadBadge(contact, 0);
    fetch(MarkReadUrl.replace('__SESSION__', encodeURIComponent(sessionId)), {
      method: 'POST',
  ...
  function lastRenderedMessageId() {
    let id = 0;
    messageStore.forEach(function(_, key){ if (Number(key) > id) id = Number(key); });
    return id;
  }

  function upsertContactFromSnapshot(data) {
    if (selectedDeviceBody && data.body !== selectedDeviceBody) return;
    let contact = document.querySelector(`[data-session-id="${data.id}"]`);
    const finalName = data.push_name && String(data.push_name).trim() !== '' ? data.push_name : data.phone_number;
    if (!contact) {
      contact = document.createElement('li');
      contact.className = 'chat-contact-list-item mb-1';
      contact.dataset.sessionId = data.id;
      contact.innerHTML = `
        <a class="d-flex align-items-center">
          <div class="flex-shrink-0 avatar"></div>
          <div class="chat-contact-info flex-grow-1 ms-4 text-truncate">
            <div class="d-flex justify-content-between align-items-center">
              <h6 class="chat-contact-name text-truncate m-0 fw-normal"></h6>
              <small class="chat-contact-list-item-time"></small>
            </div>
            <small class="chat-contact-status"></small>
          </div>
        </a>`;
      contactsList.insertBefore(contact, titleItem ? titleItem.nextSibling : contactsList.firstChild);
    }
    contact.dataset.sessionId = data.id;
    contact.dataset.sessionBody = data.body || '';
    contact.dataset.sessionPhone = data.phone_number || '';
    contact.dataset.pushName = finalName || '';
    contact.dataset.profileSender = data.profile_sender || '';
    contact.dataset.profileReceive = data.profile_receive || '';
    contact.dataset.stopAi = String(data.stop_ai || 0);
    contact.dataset.csName = data.cs_name || '';
    contact.querySelector('.chat-contact-name').textContent = finalName;
    contact.querySelector('.chat-contact-status').textContent = contactDisplayMessage(data.last_message);
    contact.querySelector('.chat-contact-list-item-time').textContent = getRelativeTime(data.updated_at);
    updateContactAvatar(contact, data.profile_sender || '', data.phone_number || '');
    updateUnreadBadge(contact, String(data.id) === String(window.currentSessionId) ? 0 : data.unread_count);
    moveContactToTop(contact);
  }

  function scheduleLiveChatSync(delay = 250) {
    if (typeof ChatSyncUrl === 'undefined') return;
    clearTimeout(liveChatSyncTimer);
    liveChatSyncTimer = setTimeout(syncLiveChat, delay);
  }

  function syncLiveChat() {
    if (typeof ChatSyncUrl === 'undefined') return;
    if (liveChatSyncInFlight) {
      liveChatSyncQueued = true;
      return;
    }
    liveChatSyncInFlight = true;
    const params = new URLSearchParams();
    if (window.currentSessionId) {
      params.set('active_session_id', window.currentSessionId);
      params.set('last_message_id', String(lastRenderedMessageId()));
    }
    fetch(ChatSyncUrl + (params.toString() ? '?' + params.toString() : ''), { headers: { 'Accept': 'application/json' } })
      .then(r => r.ok ? r.json() : Promise.reject())
      .then(data => {
        (data.sessions || []).slice().reverse().forEach(upsertContactFromSnapshot);
        (data.messages || []).forEach(m => {
          if (messageStore.has(Number(m.id))) return;
          m.profile_sender = window.currentProfileSender || m.profile_sender || '';
          m.profile_receive = window.currentProfileReceive || m.profile_receive || '';
          m.phone_number = window.currentSessionPhone || m.number || '';
          addOrAppendMessage(m);
        });
        if (window.currentSessionId) {
          updateUnreadBadge(document.querySelector(`[data-session-id="${window.currentSessionId}"]`), 0);
        }
      })
      .catch(function(){})
      .finally(() => {
        liveChatSyncInFlight = false;
        if (liveChatSyncQueued) {
          liveChatSyncQueued = false;
          scheduleLiveChatSync(150);
        }
      });
  }
  ...
  function normalizeMessage(m) {
    const next = m || {};
    next.id = next.id || `tmp-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    next.direction = next.direction || 'incoming';
    next.type = next.type || 'text';
    next.message = next.message == null ? '' : String(next.message);
  ...
  function renderMessages(messages) {
    ...
    messages.forEach(raw => {
      ...
      messageStore.set(Number(m.id) || String(m.id), m)
      ...
      try {
        ...
        const fallback = { ...m, type: 'text', message: m.message || m.original_file || m.type || '' };
        lastLi = createLi(fallback);
        ul.appendChild(lastLi);
  ...
  function showChatNotice(text) {
    const ul = document.getElementById('messages-list');
    if (!ul) return;
    ul.innerHTML = `<li class="text-center text-body-secondary py-4">${escapeHtml(text)}</li>`;
    psChatHistory?.update();
  }
  ...
  function appendToLi(li, m) {
	  ...
	  if (m.type === 'image' && m.attachment) {
		...
		const vraw = safeDecodeURIComponent(m.message);
		...
		const loc = safeParseJson(m.attachment, {});
		...
		if (!lat || !lng) {
		  ...
		  newTextDiv.innerHTML = `<p class="mb-0" style="white-space: break-spaces;">${escapeHtml(m.message || 'Location')}</p>`;
  ...
  function buildMessageGroupHtml(m, includeAvatar) {
    ...
    if (m.type === 'image' && m.attachment) {
      ...
      const lng = loc.degreesLongitude || loc.longitude;
      if (!lat || !lng) {
        contentHtml = `<div class="chat-message-wrapper flex-grow-1"><div class="chat-message-text"><p class="mb-0" style="white-space: break-spaces;">${escapeHtml(m.message || 'Location')}</p></div><div class="${isIncoming ? 'text-body-secondary' : 'text-end'} mt-1"><small>${baseTime}</small></div></div>`;
      } else {
  ...
  const contactsListClick = e => {
    try {
      ...
      if (!li || li === titleItem || li.classList.contains('chat-list-item-0')) return;
      ...
      const sessionBody  = li.dataset.sessionBody || selectedDeviceBody || '';
      const sessionPhone = li.dataset.sessionPhone || '';
      const sessionName  = li.dataset.pushName || sessionPhone;
      ...
      showChatNotice('Memuat chat...');

      if (!sessionId) {
        if (headerNameEl) headerNameEl.textContent = sessionName || 'Chat';
        if (headerPhoneEl) headerPhoneEl.textContent = sessionPhone;
        showChatNotice('Session chat belum lengkap. Sinkronisasi ulang sedang dicoba...');
        scheduleLiveChatSync(100);
        return;
      }

      if (headerNameEl) headerNameEl.textContent = sessionName;
      const stopAi = Number(li.dataset.stopAi || 0);
      window.currentStopAI = stopAi;
      updateToggleAiButton();

      if (window.currentSessionId && window.currentSessionId !== sessionId) socket.emit('leave-session', { sessionId: window.currentSessionId });
      ...
      fetch(FetchUrl.replace(':id', encodeURIComponent(sessionId)))
        .then(r => r.ok ? r.json() : Promise.reject())
        .then(messages => { renderMessages(messages); markSessionRead(sessionId); syncLiveChat(); })
        .catch(function(){
          showChatNotice('Gagal memuat isi chat. Silakan refresh halaman.');
          if (typeof notyf !== 'undefined') notyf.error('Gagal memuat isi chat. Silakan coba refresh.');
        });
    } catch (err) {
      switchToChatConversation();
      showChatNotice('Gagal membuka chat. Silakan refresh halaman.');
      if (typeof notyf !== 'undefined') notyf.error('Gagal membuka chat. Silakan refresh halaman.');
      console.error('Live chat open failed', err);
    }
  };

  contactsList.addEventListener('click', contactsListClick);

  scheduleLiveChatSync(500);
  setInterval(syncLiveChat, 3000);
  window.addEventListener('focus', function(){ scheduleLiveChatSync(100); });
  socket.on('connect', function(){ scheduleLiveChatSync(100); });
  document.addEventListener('visibilitychange', function(){ if (!document.hidden) scheduleLiveChatSync(100); });
