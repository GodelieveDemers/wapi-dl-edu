...
import { syncOutgoingMessage } from './outgoingSync.js';
      ...
      try {
        ...
        if (sendMessageResult) {
          await syncOutgoingMessage({
            ...
            status: 'success',
            note: 'single blast',
            messageId: sendMessageResult?.key?.id,
            insertHistory: false,
          })
        }
        return handleResponSendMessage(sendMessageResult, responseHandler)
      } catch (error) {
        console.error('Failed to send/sync text message:', error)
